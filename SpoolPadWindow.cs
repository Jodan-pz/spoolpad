...
public partial class SpoolPadWindow : Gtk.Window {
	...
	void StatusMessage( it.jodan.SpoolPad.BaseClasses.MessageType type, string msg, string result ) {
		Application.Invoke((s, e) => { lblStatus.Text = msg; });
	...
	void EnableCommands( bool enable ) {
		Application.Invoke((s, e) => { runCodeAction.Sensitive = newAction.Sensitive = openAction.Sensitive = saveAction.Sensitive = saveAsAction.Sensitive = quitAction.Sensitive = enable; });
	}
	...
	void OnDCInit() {
		...
		try {
			...
			Application.Invoke((s, e) => { lblConnInfo.Text = string.Empty; });
		...
		SetMouseCursor(null);
		_isRunning = false;
		Application.Invoke((s, e) => { EnableCommands(true); });
	}
